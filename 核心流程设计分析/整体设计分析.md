...
变更为当双参数消费函数（biconsumer）结束后进行任务，具体如下：

将形如 `{@link com.hazelcast.core.ExecutionCallback}` 的形式和 形如`{ @link java.util.function.BiConsumer}`的形式结合，使得最后以 ` {@link java.util.concurrent.CompletableFuture#whenComplete(BiConsumer)}`形式呈现而可以被直接使用
...
在此类中的改写 在序列化转换返回id的时候返回的是RUNNABLE_ADAPTER参数，对应hook中又调用了同名方法（序列化带来一定的繁琐行提升？）
